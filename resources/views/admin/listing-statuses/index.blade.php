...
@section('head')
    <title>Listing Statuses - {{ Settings::get('site_name') }}</title>
@endsection
...
<div class="uk-container uk-container-center uk-margin-top">
	<div class="uk-panel">
		<h3 class="uk-panel-title">Listing Statuses</h3>
	    <hr>
	    <div class="">
			...
			<table class="uk-table uk-table-hover uk-table-striped">
	            ...
	            <tbody>
	                @foreach($statuses as $status)
	                    <tr>
	                      	<td><input type="checkbox" name="checkedLine" value="{{$status->id}}"/></td>
	                        <td>{{ $status->id }}</td>
	                        <td class="uk-text-center">@if($status->published)<i class="uk-icon-check"></i>@else<i class="uk-icon-remove"></i>@endif</td>
	                        <td>{{ $status->name }}</td>
	                        <td>
	                            ...
	                            <div class="uk-button-dropdown" data-uk-dropdown>
	                                ...
	                                <div class="uk-dropdown uk-dropdown-small">
	                                    <ul class="uk-nav uk-nav-dropdown">
	                                        <li><a href="bikes/types/{{ $status->id }}">Edit</a></li>
	                                        <li><a href="bikes/types/clone/{{ $status->id }}">Clone</a></li>
	                                        <li><a id="{{ $status->id }}" onclick="deleteObject(this)">Delete</a></li>
	                                    </ul>
	                                </div>
	                            </div>
	                        </td>
		...
		@include('admin.listing-statuses.new')
	</div>
</div>
@endsection
...
@section('js')
	...
	<script type="text/javascript">
	    ...
	    function deleteObject(sender) {
	        $.post("{{ url('/admin/listing-statuses') }}/" + sender.id, {_token: "{{ csrf_token() }}", _method:"DELETE"}, function(result){
	            location.reload();
	        });
	    }
	    ...
	    function deleteObjects() {
            ...
            $.post("{{ url('/admin/listing-statuses/delete') }}", {_token: "{{ csrf_token() }}", ids: checkedValues}, function(result){
                location.reload();
            });
        }
	</script>
