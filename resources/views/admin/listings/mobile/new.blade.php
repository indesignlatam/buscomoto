	...
	<div class="uk-panel">
		...
		<form id="create_form" class="uk-form uk-form-stacked" method="POST" action="{{ url('/admin/listings') }}">
			...
			<div class="uk-form-row">
				...
				<input class="uk-width-1-1 uk-form-large" id="price" type="text" name="price" placeholder="{{ trans('admin.price') }}" value="{{ old('price') }}" onkeyup="format(this);">
			</div>

			<div class="uk-form-row uk-flex">
				<div class="">
			        <label class="uk-form-label" for="">{{ trans('admin.year') }} <i class="uk-text-danger">*</i></label>
					<input class="uk-width-1-1 uk-form-large" type="text" name="year" placeholder="{{ trans('admin.year') }}" value="{{ old('year') }}" onkeyup="format(this);">
				</div>

				<div class="uk-margin-small-left">
			        <label class="uk-form-label" for="">{{ trans('admin.license_number') }}</label>
					<input class="uk-width-1-1 uk-form-large" type="text" name="license_number" placeholder="{{ trans('admin.license_number') }}" value="{{ old('license_number') }}">
				</div>
			</div>
			...
			<div class="uk-form-row uk-flex">
				<div class="">
					...
					<input class="uk-width-1-1 uk-form-large" id="odometer" type="text" name="odometer" placeholder="{{ trans('admin.odometer') }}" value="{{ old('odometer') }}" onkeyup="format(this);">
				</div>

				<div class="uk-margin-small-left">
			        <label class="uk-form-label" for="">{{ trans('admin.engine_size') }} <i class="uk-text-danger">*</i></label>
					<input class="uk-width-1-1 uk-form-large" id="engine_size" type="text" name="engine_size" placeholder="{{ trans('admin.engine_size') }}" value="{{ old('engine_size') }}" onkeyup="format(this);">
				</div>
			</div>
			
			<div class="uk-form-row">
		        <label class="uk-form-label" for="">{{ trans('admin.color') }}</label>
				<input class="uk-width-1-1 uk-form-large" type="text" name="color" placeholder="{{ trans('admin.color') }}" value="{{ old('color') }}">
			</div>
			<!-- Informacion basica del inmueble -->
			...
			<h3 class="uk-margin-small-top">{{ trans('admin.security') }}</h3>
			<div class="uk-grid">
			...
			<h3 class="uk-text-primary uk-text-bold uk-margin-small-bottom" style="text-transform: uppercase">{{ trans('admin.listing_description') }}</h3>

			<p class="uk-margin-top-remove">{{ trans('admin.listing_description_help') }}</p>

			<textarea class="uk-width-1-1 uk-margin-small-bottom" id="description" rows="5" name="description" maxlength="2000">{{ old('description') }}</textarea>
			<!-- Informacion adicional -->

			<hr>

			<!-- Images -->
			<h3 class="uk-text-primary uk-text-bold uk-margin-small-bottom" style="text-transform: uppercase">{{ trans('admin.images') }}</h3>
			<h3 class="uk-margin-top-remove">{{ trans('admin.images_notice') }}</h3>
			<!-- Images -->
...
@if(Session::get('new_user'))
	<div id="confirmation_email_modal" class="uk-modal">
	    <div class="uk-modal-dialog">
	        ...
	        <div class="uk-modal-header uk-text-bold">
	        	{{ trans('admin.confirm_email_modal') }}
	        </div>

	        <div>
	        	<img src="{{ asset('images/support/user/welcome.jpg') }}">

	        	<h3 class="uk-text-center">{{ preg_replace('/((\w+\W*){0}(\w+))(.*)/', '${1}', Auth::user()->name) .' ' . trans('admin.welcome_new_user_title') }}</h3>
	        	<p>{{ trans('admin.welcome_new_user') }}</p>

	        	<a class="uk-button uk-button-large uk-button-success uk-modal-close uk-width-1-1" id="open" href="" target="_blank">{{ trans('admin.open') }}</a>
	        </div>
	    </div>
	</div>


	<!-- Facebook Conversion Code for Registros - BuscoMoto.co -->
	<script>(function() {
	...
	window._fbq.push(['track', '6029740676149', {'value':'5000.00','currency':'COP'}]);
	</script>
	<noscript><img height="1" width="1" alt="" style="display:none" src="https://www.facebook.com/tr?ev=6029740676149&amp;cd[value]=5000.00&amp;cd[currency]=COP&amp;noscript=1" /></noscript>
	<!-- Facebook Conversion Code for Registros - BuscoMoto.co -->
@endif
<!-- Email confirmation message sent modal -->
