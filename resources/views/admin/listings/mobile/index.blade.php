...
@section('head')
    ...
    @else
    <title>{{ trans('admin.listings') }} | {{ Settings::get('site_name') }}</title>
    @endif
@endsection
...
<div class="uk-container uk-container-center uk-margin-top">
	...
	<div class="uk-panel">	
		@if(Auth::user()->isAdmin())
		    ...
		    <div class="uk-flex">
		        <a class="uk-button uk-button-large uk-button-primary uk-width-1-1" href="{{ url('/admin/listings/create') }}">{{ trans('admin.new') }}</a>
                ...
                <div class="uk-panel uk-panel-box uk-panel-box-primary uk-margin-small-bottom">
                	<div class="uk-grid">
                		<div class="uk-width-1-3">
                			<a href="{{ url($listing->pathEdit()) }}"><img src="{{ asset(Image::url($listing->image_path(),['map_mini'])) }}"></a>
                		</div>

                		<div class="uk-width-2-3">
                			<a href="{{ url($listing->pathEdit()) }}"><b class="uk-text-primary">{{ $listing->title }}</b></a>
                			<div>
                				<span><i class="uk-icon-eye"></i> {{ $listing->views }}</span> | 
                				<span><i class="uk-icon-circle"></i> {{ $listing->points }}</span> | 
                				<span><i class="uk-icon-refresh"></i> {{ $listing->expires_at->diffInDays() }}</span>
                			</div>

                			<div class="uk-button-dropdown uk-width-1-1" data-uk-dropdown>
		                        <button class="uk-button uk-button-primary uk-width-1-1">{{ trans('admin.actions_button') }} <i class="uk-icon-caret-down"></i></button>
